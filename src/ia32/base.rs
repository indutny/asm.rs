use asm::*;

pub enum Operand {
  Empty,
  _Operation(u8),
  R(Register),
  D(DoubleRegister),
  M(Register, u32),
  Byte(u8),
  Word(u16),
  Long(u32),
  Quad(u64),
}

pub enum Register {
  eax = 0,
  ecx = 1,
  edx = 2,
  ebx = 3,
  esp = 4,
  ebp = 5,
  esi = 6,
  edi = 7
}

pub enum DoubleRegister {
  xmm0 = 0,
  xmm1 = 1,
  xmm2 = 2,
  xmm3 = 3,
  xmm4 = 4,
  xmm5 = 5,
  xmm6 = 6,
  xmm7 = 7
}

pub enum JumpCondition {
  IfZero,
  IfNotZero,
  IfOverlow,
  IfNoOverlow,
  IfEqual,
  IfNotEqual,
  IfGreater,
  IfLess,
  IfGreaterOrEqual,
  IfLessOrEqual
}

impl Register {
  pub fn val(&self) -> u8 { *self as u8 }
}

impl DoubleRegister {
  pub fn val(&self) -> u8 { *self as u8 }
}

pub trait AsmIA32Helper {
  fn emit_modrm(&mut self, r: Operand, rm: Operand);
}

pub trait AsmIA32 {
  fn nop(&mut self);
  fn int3(&mut self);
}

impl Operand {
  fn is_reg(&self) -> bool { match self { &R(_) => true, _ => false } }
  fn is_dreg(&self) -> bool { match self { &D(_) => true, _ => false } }
  fn is_mem(&self) -> bool { match self { &M(_, _) => true, _ => false } }
  fn is_rm(&self) -> bool { self.is_reg() || self.is_mem() }
  fn is_dm(&self) -> bool { self.is_dreg() || self.is_mem() }

  fn is_operation(&self) -> bool {
    match self { &_Operation(_) => true, _ => false }
  }

  fn val(&self) -> u8 {
    match self {
      &R(ref r) | &M(ref r, _) => r.val(),
      &D(ref d) => d.val(),
      &_Operation(op) => op & 7,
      _ => 0
    }
  }
}

impl<M: AsmBuffer> AsmIA32Helper for M {
  fn emit_modrm(&mut self, r: Operand, rm: Operand) {
    assert!(r.is_reg() || r.is_dreg() || r.is_operation());
    let rbit = r.val() << 3;

    match rm {
      M(ref rm, 0) => self.emitb(rbit | rm.val()),
      M(ref rm, b) if b <= 0xff => {
        self.emitb(0b0100_0000 | rbit | rm.val());
        self.emitb(b as u8)
      },
      M(ref rm, l) => {
        self.emitb(0b1000_0000 | rbit | rm.val());
        self.emitl(l)
      },
      R(ref rm) => self.emitb(0b1100_0000 | rbit | rm.val()),
      D(ref rm) => self.emitb(0b1100_0000 | rbit | rm.val()),
      Byte(_) => self.emitb(rbit),
      Word(_) => self.emitb(rbit),
      Long(_) => self.emitb(rbit),
      Quad(_) => self.emitb(rbit),
      Empty => self.emitb(rbit),
      _ => fail!()
    }
  }
}

impl<M: AsmBuffer+AsmIA32Helper> AsmIA32 for M {
  fn nop(&mut self) { self.emitb(0x90); }
  fn int3(&mut self) { self.emitb(0xcc); }
}
